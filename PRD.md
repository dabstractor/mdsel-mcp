...
* Invoke `mdsel index`
* Return compact text output exactly as emitted
* No modification
...
`mdsel` supports shorthand forms for common selectors:

| Shorthand | Full Form |
|-----------|-----------|
| `h1`-`h6` | `heading:h1`-`heading:h6` |
| `code` | `block:code` |
| `para` / `paragraph` | `block:paragraph` |
| `list` | `block:list` |
| `table` | `block:table` |
| `quote` / `blockquote` | `block:blockquote` |

Index notation:
* Dot notation: `h2.0` (preferred)
* Bracket notation: `h2[0]` (also supported)
* Ranges: `h2.1-3` or `h2[1-3]`
* Comma lists: `h2.0,2,4` or `h2[0,2,4]`
* No index: `h2` returns all h2 headings
...
* Byte-for-byte identical to `mdsel` output
...
* Explanation of selector grammar (brief, mechanical)
  * Use shorthand notation (`h2.0`) in examples, not legacy syntax (`heading:h2[0]`)
